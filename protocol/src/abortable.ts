...
//
// deferCancel()'s cancel-discard answers "what does a guest cancellation of
// an in-flight host import DO?" with the reference's prompt-cancel host: the
// subtask resolves CANCELLED_BEFORE_RETURNED and the promise's eventual
// settlement is discarded. That is a statement about DELIVERY only — the
// host operation itself keeps running. A discarded socket connect keeps
// connecting, a discarded timer keeps its callback armed, a discarded fetch
// keeps streaming bytes nobody will ever read.
//
// abortable() is the third mark in the family, alongside `suspending()` and
// `deferCancel()`, and it closes exactly that gap by handing the host the
// platform's own cancellation vocabulary. Every call of a marked import
// receives a fresh
...
//     cancellation that took the cancel-discard, and for nothing else.
//     Instance teardown does not abort in-flight calls (future work).
//
...
// partial value) lands on deferCancel()'s resolved-subtask guards and is discarded like
...
// than a wart: a sync-typed import's suspending() park never mints a subtask handle, so
...
// all three may sit on one function. Like deferCancel() and unlike
// suspending() this brand is not mode evidence, so there is no
// `anyAbortableImport` analogue — it is read
 ...
 * of merely stopping its delivery (deferCancel() discards the result; the
 * operation runs on).
 *
 ...
 * descriptor. Constructors are never markable (synchronous per §"Resources";
 * the language reserves no constructor-decorator position anyway).
 *
