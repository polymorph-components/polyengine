...
Deno.test({
  ...
  fn: async () => {
    ...
    assertEq(
      Cell.disposed,
      [7],
      "the guest dropping its owning registration runs [Symbol.dispose]",
    );
...
Deno.test("host resources: registrations do not use backing-object identity", () => {
  const registry = new HostResourceRegistry("Cell");
  Cell.disposed = [];
  const cell = new Cell(7);
  const first = registry.repFor(cell);
  const second = registry.repFor(cell);
  const borrowA = registry.borrowFor(cell);
  const borrowB = registry.borrowFor(cell);
  assertEq(new Set([first, second, borrowA.rep, borrowB.rep]).size, 4);
  registry.dtor(first);
  assertEq(registry.lookup(second), cell);
  assertEq(registry.lookup(borrowA.rep), cell);
  assertEq(Cell.disposed, [7]);
  borrowA.release();
  borrowA.release();
  assertEq(registry.lookup(borrowB.rep), cell);
  assertEq(registry.release(second), cell);
  borrowB.release();
  assertEq(registry.liveCount, 0);
});

Deno.test({
  name: "host resources: same backing object creates independent guest owns",
  ignore: !overlapReady,
  fn: async () => {
    ...
    Cell.disposed = [];
    const cell = new Cell(7);
    const first = await c.exports.hold(cell);
    const second = await c.exports.hold(cell);
    assertEq(
      first === second,
      false,
      "guest handles are independently allocated",
    );
    await c.exports.dropHeld(first);
    assertEq(Cell.disposed, [7]);
    assertEq(await c.exports.heldValue(second), 7);
    await c.exports.dropHeld(second);
    assertEq(Cell.disposed, [7, 7]);
  },
});

Deno.test({
  name:
    "host resources: returning one own preserves a same-object registration",
  ignore: !overlapReady,
  fn: async () => {
    const c = await instantiateFixture(overlapFixture, {
      "host:api/res": { R: Cell, value: (r: Cell) => r.v },
    ...
    Cell.disposed = [];
    const cell = new Cell(7);
    const returnedHandle = await c.exports.hold(cell);
    const remainingHandle = await c.exports.hold(cell);
    assertEq(await c.exports.returnHeld(returnedHandle), cell);
    assertEq(await c.exports.heldValue(remainingHandle), 7);
    assertEq(Cell.disposed, []);
    await c.exports.dropHeld(remainingHandle);
    assertEq(Cell.disposed, [7]);
  },
});

Deno.test({
  name: "host resources: overlapping borrows and own creation are independent",
  ignore: !overlapReady,
  fn: async () => {
    ...
    assertEq(registry.liveCount, 2, "each call owns one temporary mapping");
    const held = await c.exports.hold(cell);
    assertEq(
      registry.liveCount,
      3,
      "own registration is separate from borrows",
    );
    resolvers[0]();
    assertEq(await first, 7);
    assertEq(registry.liveCount, 2);
    resolvers[1]();
    assertEq(await secondOutcome, undefined);
    assertEq(await second, 7);
    assertEq(registry.liveCount, 1);
    assertEq(Cell.disposed, [], "borrow cleanup never disposes backing data");
...
Deno.test({
  name: "host resources: temporary registrations release on call failure",
  ignore: !overlapReady,
  fn: async () => {
    const c = await instantiateFixture(overlapFixture, {
      "host:api/res": { R: Cell, value: (r: Cell) => r.v },
    ...
    const conversionError = await caught(() => c.exports.acceptTwo(cell, 0));
    assertEq(conversionError instanceof TypeError, true);
    assertEq(
      registry.liveCount,
      0,
      "earlier arguments unwind on conversion failure",
    );

    const guestError = await caught(() => c.exports.fail(cell));
    assertEq(guestError instanceof Trap, true);
    assertEq(
      registry.liveCount,
      0,
      "arguments unwind when guest execution fails",
    );
  },
});
...
Deno.test({
  ...
  ignore: !borrowReady,
  fn: async () => {
    // A host-originated borrow is always call-scoped. It neither reuses nor
    // removes an independent own registration backed by the same object.
    const c = await instantiateFixture(
      "runtime/tests/embedder/host-borrow.wasm",
      { "host:api/res": { R: Cell, value: (r: Cell) => r.v } },
    );
